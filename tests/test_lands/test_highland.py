...
def test_create_highland(reset_params):
    """
    Testing create an Instance of Highland class object
    ...
    Notes
    ------
    - This creates a Highland instance
    - asserts fodder is equal to 300
    Returns
    -------
    area.fodder == 300 is True

    """
    area = HighLand()
    assert area.fodder == 300


@pytest.mark.parametrize("f_max", [0, 0.1, 200, 201.15])
def test_update_params_success(f_max, reset_params):
    ...
    Notes
    ------
    - Sets f_max to 300 with the set_land_params function
    - asserts fodder and f_max are 300
    Returns
    -------
    area.fodder == f_max is True
    area.f_max == f_max is True

    """
    HighLand.set_land_params({"f_max": f_max})
    area = HighLand()
...
def test_update_params_dict_fail(reset_params):
    ...
    """
    with pytest.raises(ValueError):
        HighLand.set_land_params({"ff_max": 100})
...
class TestHighLand:
    """
    The test class for the Highland object
    """

    @pytest.fixture(autouse=True)
    def create(self):
        """
        Create a Highland object for the test class

        Notes
        -----
        - Create a Highland object
    ...
    def test_death_cycle_dead(self, reset_params, mocker):
        ...
        len(self.terra.migrate_pop_herbivore) == 0 is True
        len(self.terra.migrate_pop_carnivore) == 0 is True
        ...
        assert len(self.terra.migrate_pop_herbivore) == 0
        assert len(self.terra.migrate_pop_carnivore) == 0
    ...
    def test_death_cycle_alive(self, reset_params, mocker):
        ...
        len(self.terra.migrate_pop_herbivore) == current_migrate_herb
        len(self.terra.migrate_pop_carnivore) == current_migrate_carn
        """
