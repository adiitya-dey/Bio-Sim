...
@pytest.mark.parametrize("age, weight", [(-1, 8.0),
                                         (14, -1.0)])
def test_create_negative(age, weight, re_update_params):
    """
...
def test_create_with_age_none(age, weight, re_update_params):
    ...
    Returns
    -------
    assert carn1.age == 0  is True
    assert carn1.weight == weight is True
    """
    carn1 = Carnivore(age, weight)
    assert carn1.age == 0
    assert carn1.weight == weight
...
def test_set_params_success(key, value, re_update_params):
    ...
    Returns
    -------
    Carnivore.params[key] == value is True

    """
    Carnivore.update_params({key: value})
    assert Carnivore.params[key] == value
...
@pytest.mark.parametrize("key, value", [("DeltaPhiMax", 1),
                                        ("DeltaPhiMax", 0.1)])
def test_set_params_delta_phi_max_success(key, value, re_update_params):
    """
...
def test_update_params(parameters, re_update_params):
    ...
    Returns
    -------
    Carnivore.params["eta"] == parameters["eta"] is True
    Carnivore.params["w_birth"] == parameters["w_birth"] is True
    """
    # params = parameters
    Carnivore.update_params(parameters)

    assert Carnivore.params["eta"] == parameters["eta"]
    assert Carnivore.params["w_birth"] == parameters["w_birth"]


@pytest.mark.parametrize("age, weight", [(1, 10.0),
                                         (5, 20.0),
                                         (30, 50),
                                         (35, 60)])
class TestCarnivore:
    """
    Carnivore Class Testing
    """

    @pytest.fixture(autouse=True)
    def create(self, age, weight):
        """
        Testing Carnivore creation
        ...
        - We create 4 carnivores

        - With ages and weight set using parametrize

        ..code::

                @pytest.mark.parametrize("age, weight", [(1, 10.0),(5, 20.0),(30, 50),(35, 60)])
    ...
    def reset_params(self):
        ...
        """
        Testing carnivore class parameter update
    ...
    def test_create(self):
        ...
        """
        Testing age and weight values for created carnivore
        ...
        Returns
        -------
        self.carn.age == self.age is True
        self.carn.weight == self.weight is True


        """
        assert self.carn.age == self.age
        assert self.carn.weight == self.weight

    @pytest.mark.parametrize("years", [1, 10, 23, 40])
    def test_aging(self, years):
        """
        Testing Carnivore aging
        ...
        Returns
        -------
        self.carn.age == self.age + years (True)
        """
        for _ in range(years):
            self.carn.aging()

        assert self.carn.age == self.age + years

    @pytest.mark.parametrize("years", [1, 7])
    @pytest.mark.parametrize("beta", [0.6, 1.2])
    @pytest.mark.parametrize("food", [0.0, 25.0, 50.0, 75.0])
    def test_increase_weight(self, food, beta, years, reset_params):
        """
        Testing Carnivore Increase weight function
        ...
        - We check with the values to ensure the carnivore increases weight
        ...
        """
        self.carn.update_params({'beta': beta})

        for _ in range(years):
            self.carn.increase_weight(food)
    ...
    def test_decrease_weight(self, eta, years, reset_params):
        """
        Testing carnivore decrease weight function
        ...
        ..code::
              ...
              Carnivore.update_params({'eta': eta})
    ...
    def test_fitness(self, phi_age, a_half, phi_weight, w_half, reset_params):
        ...
        - update carnivore parameters with values
        ...
        Returns
        -------
        0 <= self.carn.phi <= 1 is True

        self.carn.phi == c_phi is True
        """
        ...
        assert 0 <= self.carn.phi <= 1
        assert self.carn.phi == c_phi

    def test_death_alive(self, mocker, reset_params):
        r"""
        Testing Carnivore death status == alive
        ...
        - assert that all carnivores do not die
        ...
        self.carn.death() is False


        """
        mocker.patch('random.random', return_value=0.8)
        assert self.carn.death() is False

    def test_death_dead(self, mocker, reset_params):
        r"""
        Testing carnivore death status == dead
        ...
        - assert that all carnivores  die
        ...
        assert self.carn.death()

    @pytest.mark.parametrize("h_age, h_weight", [(30, 10.0), (36, 5.0)])
    def test_carnivore_prob_kill(self, h_age, h_weight, reset_params):
        """
        Testing prob_kill function
        ...
        Notes
        ------
        - Parse herbivore age and weight and return fitness
        - Assert probability of carnivore killing herbivore
        is between 0 and 1

        ..code::


                    if self.phi <= herbivore_fitness:
        ...
        Returns
        -------
        1 >= self.carn.prob_kill(herbivore.phi) >= 0 is True
        """
        herbivore = Herbivore(h_age, h_weight)

        assert 1 >= self.carn.prob_kill(herbivore.phi) >= 0

    @pytest.mark.parametrize("h_pop", [[Herbivore(40, 5) for _ in range(2)]
                                       + [Herbivore(10, 30) for _ in range(5)]])
    def test_carnivore_feeding(self, h_pop, mocker):
        """
        Testing carnivore feeding
        ...
        Notes
        ------
        - Set Mocker value to 0.05

        - Parse h_pop into carnivore feeding function

        - list of animals not eaten < h_pop

        Returns
        -------
        len(new_list) < len(h_pop) is True
        """
        mocker.patch("random.random", return_value=0.05)
        new_list = self.carn.feeding(h_pop)
        assert len(new_list) < len(h_pop)

    def test_migration_prob_parametric_success(self, reset_params):
        """
        Testing carnivore migration probability with Mu
        ...
        Notes
        ------
        - We control the value of Mu to 1.5
        - This ensures all animals have probability greater than random value
        ..code::
        ...
        - We assert that all animals will return migrate True


        Returns
        -------
        self.carn.migrate_prob == True

        """
        Carnivore.update_params({"mu": 1.5})
        assert self.carn.migrate_prob()

    def test_migration_prob_parametric_fail(self, reset_params):
        """
        ...
        Notes
        ------
        - We control the value of Mu to 0.01
        ...
        - We assert that all animals will return migrate false

        Returns
        -------
        self.carn.migrate_prob() is False

        """
        Carnivore.update_params({"mu": 0.01})
        assert self.carn.migrate_prob() is False

    def test_migration_prob_fail(self, mocker, reset_params):
        """
        Testing migration probability failure
        ...
        ..code::
                ...
                if random.random() < probability:
                    ...
                    return False
        - We assert that the carnivores return False for migration
    ...
    def test_stat_migration(self, reset_params):
        ...
        mu = self.carn.params['mu']
        migration_probability = mu * self.carn.phi

        num_migrants = sum(self.carn.migrate_prob()
    ...
    def test_stat_death(self, reset_params):
        ...
        omega = self.carn.params['omega']
        death_probability = omega * (1 - self.carn.phi)

        num_deaths = sum(self.carn.death() for _ in range(num_trials))  # True == 1, False == 0
...
@pytest.mark.parametrize("age, weight", [(30, 23.0),
                                         (10, 33.0),
                                         (10, 0.0)])
def test_birth_no_child_carnivore(age, weight, mocker, re_update_params):
    """
    Testing Carnivore does not give birth
    ...
    - for the animal with weight 23, this is less than the minimum weight 24
    ...
    - For the animal with weight 33, with the birth weight of the child at 30
    ...
    - The last animal with a weight of 0 would also be thrown out.
    ...
    """
    # minimum_weight => 23 < 24 <- zeta * (w_birth + sigma_birth)
    # weight_loss => 33 <- xi * birth_weight
    # zero_weight => 0
    mocker.patch('random.lognormvariate', return_value=30.0)
    assert Carnivore(age, weight).birth() is None


@pytest.mark.parametrize("age, weight", [(10, 25.0),
                                         (30, 40.0)])
def test_birth_child_born(age, weight, mocker, re_update_params):
    """
    Testing carnivore meets child_birth conditions
    ...
    - For the first animal we set the weight at 25
    ...
    """
    mocker.patch('random.lognormvariate', return_value=22)
    assert isinstance(Carnivore(age, weight).birth(), Carnivore)
